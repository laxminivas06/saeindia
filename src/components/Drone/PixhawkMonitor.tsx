...
import { Radio, Usb, Activity, Cpu, Terminal, AlertTriangle, Info, CheckCircle2 } from 'lucide-react';
...
export const PixhawkMonitor: React.FC<PixhawkMonitorProps> = ({
  ...
  const handleConnectSerial = async () => {
    setConnecting(true);
    await mavlinkService.connectWebSerial();
    setConnecting(false);
  };
    ...
    <div className={`bg-slate-900/90 rounded-xl p-3.5 sm:p-4 border border-slate-800 hud-border font-mono ${className}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs mb-3">
        ...
        <div className="bg-slate-950/60 p-2 rounded border border-slate-800">
          ...
          <div className="font-bold text-emerald-400 text-[10px] truncate">Pixhawk 2.4.8</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-800/80">
        <div className="text-[10px] text-slate-400 flex items-center space-x-1.5">
          ...
          <span>Heartbeat: {connectionState.lastHeartbeat ? `${Math.max(0, Math.round((Date.now() - connectionState.lastHeartbeat) / 1000))}s ago` : 'None'}</span>
        </div>
        ...
        {!connectionState.isConnected && (
          <button
            ...
            title="Connect Physical Pixhawk via USB OTG / Serial"
          >
